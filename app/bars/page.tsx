...
export default async function BarsPage({
  ...
  const reviews = allReviews
    .filter((r) => !r.isDraft && r.contentType === 'review' && r.venueType === 'bar')
    ...
    <div className="min-h-screen bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="mb-8">
          ...
          <p className="mt-2 text-slate-600">Cocktails, wine, breweries, and more.</p>
        </div>
        ...
        {reviews.length > PAGE_SIZE && (
          <div className="mt-12 flex justify-center">
            <div className="flex items-center gap-2">
              <Link
                href={safePage <= 1 ? '/bars' : `/bars?page=${safePage - 1}`}
              ...
              <Link
                href={safePage >= totalPages ? `/bars?page=${totalPages}` : `/bars?page=${safePage + 1}`}
