...
export default async function CafesPage({
  ...
  const reviews = allReviews
    .filter((r) => !r.isDraft && r.contentType === 'review' && r.venueType === 'cafe')
        ...
        {reviews.length > PAGE_SIZE && (
          <div className="mt-12 flex justify-center">
            <div className="flex items-center gap-2">
              <Link
                href={safePage <= 1 ? '/cafes' : `/cafes?page=${safePage - 1}`}
              ...
              <Link
                href={safePage >= totalPages ? `/cafes?page=${totalPages}` : `/cafes?page=${safePage + 1}`}
